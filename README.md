# 👋 Hi, I'm Sanjay Singh  

🚀 **Full Stack Developer**  
🎓 B.Tech CSE (IT) | Sage University, Indore  
💻 Passionate about building scalable web applications & real-time systems  

---

## 🧠 Professional Summary  

Full Stack Developer with hands-on experience in **MERN stack**, and **DevOps tools**.  
Skilled in building **scalable APIs**, **real-time systems**, and **production-ready deployments**.  
Looking for opportunities to contribute as a **Software / Full Stack Developer**.

---

## 🛠️ Tech Stack  

### 💻 Languages  
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?logo=javascript&logoColor=black&style=for-the-badge)
![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white&style=for-the-badge)

### 🎨 Frontend  
![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=black&style=for-the-badge)
![Next.js](https://img.shields.io/badge/-Next.js-black?logo=next.js&logoColor=white&style=for-the-badge)
![Tailwind CSS](https://img.shields.io/badge/-TailwindCSS-38B2AC?logo=tailwind-css&logoColor=white&style=for-the-badge)

### 🧩 Backend  
![Node.js](https://img.shields.io/badge/-Node.js-339933?logo=node.js&logoColor=white&style=for-the-badge)
![Express.js](https://img.shields.io/badge/-Express.js-000000?logo=express&logoColor=white&style=for-the-badge)
![Prisma](https://img.shields.io/badge/-Prisma-2D3748?logo=prisma&logoColor=white&style=for-the-badge)

### 🗄️ Databases  
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?logo=mongodb&logoColor=white&style=for-the-badge)
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?logo=mysql&logoColor=white&style=for-the-badge)

---

## 🚀 Projects  

### 🔥 Exchange System (Real-Time Trading Platform)  
**Tech:** Next.js, Node.js, WebSocket 

- Built real-time trade matching engine  
- Live price updates & order book using WebSockets   
- Time-series database for transaction history  

---

### ⚙️ Result Scraper System  
**Tech:** Node.js, Express, Prisma  

- Automated college result scraping  
- Type-safe backend architecture  
- REST APIs for database integration 

---

### ✍️ Blog Website (MERN Stack)  
**Tech:** React, Node.js, MongoDB, JWT  

- Secure authentication system  
- Blog search & filtering  
- Fully responsive UI  
- CRUD APIs with MongoDB  

---

## 🧠 Other Skills  

- Data Structures & Algorithms (DSA)  
- Cloud Basics  
- SEO, Google Analytics  
- WordPress & Affiliate Marketing  
- Networking (Juniper Certified)  

---

## 🌐 Connect With Me  

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?logo=linkedin&logoColor=white&style=for-the-badge)](https://www.linkedin.com/in/iamsanjaysingh)  
[![Email](https://img.shields.io/badge/-Email-D14836?logo=gmail&logoColor=white&style=for-the-badge)](mailto:sanjaysinghmp09@gmail.com)

---

## 🎯 Open to Opportunities  

Actively looking for **Software Engineer / Full Stack Developer** roles.  
💡 Let’s build scalable & impactful products together.

---

## 📊 GitHub Statistics  

![Profile Summary](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=iamsanjaysingh&theme=radical)

![Stats](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=iamsanjaysingh&theme=radical)
